...
.b-confirmed  { background:var(--tag-green-bg); color:var(--tag-green-fg); }
.b-checked_in { background:var(--tag-blue-bg); color:var(--tag-blue-fg); }
.b-checked_out{ background:var(--tag-slate-bg); color:var(--tag-slate-fg); }
...
@media(max-width:560px) {
    ...
    .booking-action { width:100%; flex-direction:row !important; justify-content:flex-end; }
    .property-grid { grid-template-columns:repeat(auto-fill,minmax(150px,1fr)); }
}
...
@section('content')
    ...
    <div id="rt-fd-banner">
        <span><i class="bi bi-arrow-repeat me-1"></i> Another staff member made changes here — refresh to see the latest.</span>
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            <div class="stat-icon tag-red"><i class="bi bi-people-fill"></i></div>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="booking-row">
                    ...
                    <div class="booking-action" style="display:flex; gap:8px; flex-direction:column; align-items:flex-end;" data-checkout-row="{{ $booking->id }}">
                        <form method="POST" action="{{ route('staff.checkout', $booking) }}" id="checkoutForm_{{ $booking->id }}" class="checkout-form-{{ $booking->id }}">
                            @csrf @method('PATCH')
    ...
    <div class="tab-content" id="tab-guests">
        <div class="card">
            <div class="card-head">
                ...
                <span class="text-muted-theme" style="font-size:12px;">{{ $currentGuests->count() }} guest{{ $currentGuests->count()!=1?'s':'' }}</span>
            </div>
            <div class="card-body">
                ...
                <div class="booking-row">
                    ...
                    <div class="booking-action" style="display:flex;flex-direction:column;gap:8px;align-items:flex-end;">
                        ...
                        <form method="POST" action="{{ route('staff.checkout', $booking) }}" id="checkoutFormGuests_{{ $booking->id }}" class="checkout-form-{{ $booking->id }}">
                            @csrf @method('PATCH')
        ...
        <div class="card mb-3">
            ...
            <div class="card-body">
                ...
                <div class="task-row">
                    ...
                    <div>
                        <form method="POST" action="{{ route('staff.tasks.complete', $task) }}" id="taskCompleteForm_{{ $task->id }}" class="task-form-{{ $task->id }}">
                            @csrf @method('PATCH')
            ...
            <div class="card-body">
                ...
                <div class="task-row">
                    ...
                    <div style="display:flex;gap:6px;" class="task-actions-{{ $task->id }}">
                        <form method="POST" action="{{ route('staff.tasks.start', $task) }}" id="taskStartForm_{{ $task->id }}" class="task-form-{{ $task->id }}">
                            @csrf @method('PATCH')
...
<div id="checkinConfirmModal" class="modal-overlay" style="display:none;">
    <div class="modal-box" style="width:440px;">
        <div class="modal-head">
            <div>
                <div class="modal-title">May Natitirang Balance</div>
                <div style="color:rgba(255,255,255,.5);font-size:12px;margin-top:2px;">Guest: <span id="ccGuestName"></span></div>
            </div>
            <button type="button" onclick="closeCheckinConfirmModal()" class="modal-close">✕</button>
        </div>
        <div class="modal-body">
            <div style="background:#fef2f2;border:1px solid #fecaca;border-radius:10px;padding:12px 16px;margin-bottom:18px;display:flex;justify-content:space-between;align-items:center;">
                <span style="font-size:13px;color:#991b1b;">Balance Due</span>
                <span style="font-family:'Playfair Display',serif;font-size:20px;font-weight:700;color:#dc2626;" id="ccBalance">₱0.00</span>
            </div>

            <p style="font-size:12.5px;color:#6B7A8D;margin-bottom:16px;line-height:1.6;">
                Pumili ng isasagawa bago matuloy ang check-in ng guest na ito:
            </p>
            ...
            <button type="button" onclick="ccPayNow()"
                ...
                <div class="d-flex-gap-10">
                    <div style="width:32px;height:32px;border-radius:8px;background:#16a34a;color:#fff;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;">
                        <i class="bi bi-cash-stack"></i>
                    </div>
                    <div>
                        <div style="font-size:13px;font-weight:600;color:#15803d;">Bayaran Ngayon</div>
                        <div style="font-size:11.5px;color:#6B7A8D;margin-top:1px;">Buksan ang Payment form para i-record ang buong balance</div>
                    </div>
                </div>
            </button>
            ...
            <div style="background:#fffbeb;border:1.5px solid #fde68a;border-radius:10px;padding:14px 16px;">
                <div class="d-flex-gap-10" style="margin-bottom:10px;">
                    <div style="width:32px;height:32px;border-radius:8px;background:#d97706;color:#fff;display:flex;align-items:center;justify-content:center;font-size:14px;flex-shrink:0;">
                        <i class="bi bi-clock-history"></i>
                    </div>
                    <div>
                        <div style="font-size:13px;font-weight:600;color:#92400e;">Ipagpaliban (Deferred)</div>
                        <div style="font-size:11.5px;color:#6B7A8D;margin-top:1px;">Babayaran bago mag-check-out</div>
                    </div>
                </div>
                <label style="display:flex;align-items:flex-start;gap:8px;font-size:12px;color:#374151;cursor:pointer;margin-bottom:10px;">
                    <input type="checkbox" id="ccDeferCheckbox" style="margin-top:2px;">
                    <span>Kumpirmado kong papayagan ang guest na mag-check-in ngayon at babayaran na lang ang natitirang balance bago mag-check-out. Naiitala ito sa staff log.</span>
                </label>
                <button type="button" id="ccConfirmBtn" onclick="ccConfirmDeferred()" disabled
                    ...
                    <i class="bi bi-box-arrow-in-right me-1"></i> Kumpirmahin at I-check In
                </button>
            </div>
        </div>
    </div>
...
function openCheckinConfirmModal(bookingId, guestName, balanceDue) {
    ...
    document.getElementById('ccBalance').textContent = '₱' + parseFloat(balanceDue).toLocaleString('en-PH', {minimumFractionDigits:2});
