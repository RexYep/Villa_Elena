...
.filter-btn{padding:7px 16px;border-radius:20px;font-size:12px;font-weight:500;border:1.5px solid var(--border);color:var(--muted);background:#fff;cursor:pointer;transition:.2s;}
.filter-btn:hover,.filter-btn.active{background:var(--stone);color:#fff;border-color:var(--stone);}

/* ================= BOOKING CARD ================= */
.booking-card{background:#fff;border-radius:16px;border:1px solid var(--border);overflow:hidden;margin-bottom:14px;transition:.2s;}
...
.bc-img,.bc-img-placeholder{width:68px;height:68px;border-radius:12px;flex-shrink:0;}
.bc-img{object-fit:cover;background:var(--sand);}
.bc-img-placeholder{background:var(--sand);display:flex;align-items:center;justify-content:center;font-size:24px;color:var(--muted);}
...
.bc-dates{font-size:12px;color:var(--muted);margin-top:4px;display:flex;align-items:center;gap:6px;flex-wrap:wrap;}
.bc-right{text-align:right;flex-shrink:0;}
.bc-amount{margin-top:7px;font-weight:700;font-size:15px;font-family:'Playfair Display',serif;}
...
@media (max-width:768px){
    .booking-card-inner{flex-direction:column;align-items:flex-start;}
    .bc-right{text-align:left;width:100%;}
}
</style>
@endpush
