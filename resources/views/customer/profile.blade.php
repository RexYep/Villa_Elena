...
.form-card{background:#fff;border-radius:16px;border:1px solid var(--border);overflow:hidden;margin-bottom:24px;}
.form-card-head{padding:18px 22px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:10px;}
...
.form-control,.form-select{padding:11px 14px;}
...
.avatar-preview{width:72px;height:72px;border-radius:50%;object-fit:cover;background:var(--sand);display:flex;align-items:center;justify-content:center;font-size:26px;font-weight:600;color:var(--stone);flex-shrink:0;}

.btn-submit{background:var(--stone);color:#fff;border:none;border-radius:10px;padding:13px 20px;font-size:14px;font-weight:600;cursor:pointer;font-family:'Jost',sans-serif;transition:all .2s;display:inline-flex;align-items:center;justify-content:center;gap:8px;}
.btn-submit:hover{background:var(--gold);color:var(--stone);}

.danger-card{border:1px solid #fecaca;background:#fef2f2;border-radius:16px;overflow:hidden;}
.danger-card .form-card-head{border-bottom:1px solid #fecaca;}
.btn-danger{background:#dc2626;color:#fff;border:none;border-radius:10px;padding:13px 20px;font-size:14px;font-weight:600;cursor:pointer;font-family:'Jost',sans-serif;}
...
.toggle-switch input:checked + .toggle-slider{background:var(--stone);}
.toggle-switch input:checked + .toggle-slider:before{transform:translateX(20px);}

.device-row{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 0;border-bottom:1px solid #f4efe6;}
.device-row:last-child{border-bottom:none;}
.device-name{font-size:13px;font-weight:600;color:#374151;}
.device-meta{font-size:11px;color:var(--muted);margin-top:2px;}
.this-device-tag{background:#dcfce7;color:#15803d;font-size:10px;font-weight:600;padding:2px 8px;border-radius:8px;margin-left:6px;}
.btn-remove-device{background:none;border:none;color:var(--muted);cursor:pointer;font-size:12px;padding:4px 8px;}
.btn-remove-device:hover{color:#dc2626;}
.empty-hint{font-size:12px;color:var(--muted);padding:8px 0;}
...
@section('content')
        ...
        <div class="form-card">
            ...
            <div class="form-card-body">
                ...
                <div class="avatar-row">
                    ...
                    @endif
                    <div>
                        <input type="file" name="avatar" accept="image/png,image/jpeg,image/webp" class="form-control @error('avatar') is-invalid @enderror">
                    ...
                    <div>
                        ...
                        <select name="id_type" class="form-select">
                            ...
                            @foreach(["Driver's License","Passport","SSS ID","PhilHealth ID","Voter's ID","National ID","PRC ID","Postal ID"] as $idType)
                                <option value="{{ $idType }}" {{ old('id_type', $user->id_type) == $idType ? 'selected' : '' }}>{{ $idType }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
    ...
    <div class="form-card">
        ...
        <div class="form-card-body">
            <div class="toggle-row" style="margin-bottom:{{ $user->two_factor_enabled ? '16px' : '0' }};">
                <div>
                    ...
                    <div class="hint" style="margin-top:3px;">
                        Kapag naka-on, kakailanganin mo munang i-verify ang bagong device sa pamamagitan ng code
                        na ipapadala sa email mo bago ka makapag-login dito. Hindi na kailangan ulit ito sa parehong device.
                    </div>
                </div>

                @if($user->two_factor_enabled)
                    <label class="toggle-switch">
                        <input type="checkbox" checked onclick="event.preventDefault(); document.getElementById('disable2faForm').style.display='block';">
            ...
            @if($user->two_factor_enabled)
                <form method="POST" action="{{ route('customer.profile.2fa.toggle') }}" id="disable2faForm" style="display:none;background:#f9f5ee;border-radius:10px;padding:14px;">
                    @csrf @method('PUT')
                    <label class="form-label">Enter your password to turn this off</label>
                    <input type="password" name="password" class="form-control @error('password') is-invalid @enderror" style="margin-bottom:10px;">
                    @error('password')<div class="field-error" style="margin-bottom:8px;">{{ $message }}</div>@enderror
                    <button type="submit" class="btn-danger" style="padding:9px 16px;font-size:13px;">Turn Off</button>
                </form>
            @endif
        </div>
    </div>
            ...
            @forelse($trustedDevices as $device)
                <div class="device-row">
                    ...
                    <form method="POST" action="{{ route('customer.profile.devices.destroy', $device) }}"
                        ...
                        @csrf @method('DELETE')
                        <button type="submit" class="btn-remove-device"><i class="bi bi-trash"></i> Remove</button>
                    </form>
                </div>
            @empty
