...
.review-card{background:#fff;border-radius:16px;border:1px solid var(--border);padding:20px 22px;margin-bottom:16px;}
.review-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:8px;}
.review-property{font-family:'Playfair Display',serif;font-size:16px;font-weight:600;}
...
.review-content{font-size:13px;color:#374151;line-height:1.6;}
.review-actions{display:flex;gap:10px;margin-top:14px;}
.btn-edit,.btn-delete{border:none;border-radius:8px;padding:8px 16px;font-size:12px;font-weight:600;cursor:pointer;font-family:'Jost',sans-serif;text-decoration:none;display:inline-flex;align-items:center;gap:6px;}
.btn-edit{background:#f1f5f9;color:#374151;}
.btn-edit:hover{background:#e2e8f0;}
.btn-delete{background:#fef2f2;color:#dc2626;}
.btn-delete:hover{background:#fee2e2;}

.admin-reply{background:#f9f5ee;border-radius:10px;padding:12px 14px;margin-top:12px;font-size:12px;}
.admin-reply strong{display:block;color:var(--stone);margin-bottom:3px;}
    ...
    @forelse($reviews as $review)
        <div class="review-card">
            <div class="review-head">
                ...
                @endphp
                <span style="background:{{ $badge[0] }};color:{{ $badge[1] }};padding:4px 12px;border-radius:10px;font-size:11px;font-weight:600;white-space:nowrap;">{{ $badge[2] }}</span>
            </div>
