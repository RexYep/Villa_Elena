...
@section('topbar-right')
    <a href="{{ route('admin.bookings.index') }}" class="text-muted-theme" style="display:flex;align-items:center;gap:6px;text-decoration:none;font-size:13px;border:1px solid var(--border);padding:7px 14px;border-radius:9px;background:#fff;">
...
/* Cards */
.card-panel{background:var(--cream);border-radius:14px;border:1px solid var(--border);overflow:hidden;margin-bottom:20px;}
...
.card-header-custom h3{font-family:'Cormorant Garamond',serif;font-size:17px;font-weight:600;color:var(--text-main);}
.card-body-custom{padding:20px 24px;}

/* Booking Header Card */
.booking-header{background:var(--terracotta);border-radius:14px;padding:24px 28px;margin-bottom:20px;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px;}
.booking-ref{font-family:'Cormorant Garamond',serif;color:var(--gold-light);font-size:28px;font-weight:700;}
.booking-source{color:rgba(255,255,255,0.6);font-size:12px;margin-top:3px;text-transform:uppercase;letter-spacing:1px;}
...
.btn-noshow     {background:var(--stone);color:#fff;}
...
.pay-table th{font-size:11px;font-weight:600;text-transform:uppercase;letter-spacing:.7px;color:var(--muted);padding:8px 0;border-bottom:1px solid var(--border);}
.pay-table td{padding:10px 0;border-bottom:1px solid var(--border);vertical-align:middle;color:var(--text-main);}
.pay-table tr:last-child td{border-bottom:none;}

/* Form inside card */
.form-label-sm{font-size:12px;font-weight:600;color:var(--text-main);margin-bottom:5px;display:block;}
.form-control-sm-custom{border:1.5px solid var(--border);border-radius:7px;padding:8px 12px;font-size:13px;font-family:'DM Sans',sans-serif;width:100%;background:#fff;color:var(--text-main);}
.form-control-sm-custom:focus{outline:none;border-color:var(--terracotta);}
.btn-record{background:var(--terracotta);color:#fff;border:none;border-radius:8px;padding:10px;font-size:13px;font-weight:600;width:100%;cursor:pointer;font-family:'DM Sans',sans-serif;margin-top:4px;transition:background .2s;}
.btn-record:hover{background:var(--gold);}
...
@media (max-width: 480px) {
    ...
    .booking-header{justify-content:center;text-align:center;}
}
</style>
@endpush
    ...
    <div class="booking-header">
        ...
        <div class="text-center">
            <div class="booking-dates">
                ...
                Check-in @if($booking->check_in_time) · {{ \Carbon\Carbon::parse($booking->check_in_time)->format('g:i A') }} @endif
            </div>
        </div>
        ...
        <div class="text-center">
            <div class="booking-dates">
                ...
                Check-out @if($booking->check_out_time) · {{ \Carbon\Carbon::parse($booking->check_out_time)->format('g:i A') }} @endif
            </div>
        </div>
        <div class="text-center">
            ...
            <div class="card-panel">
                ...
                <div class="card-body-custom">
                    ...
                    <div class="status-actions">
                        ...
                        <form method="POST" action="{{ route('admin.bookings.status', $booking) }}" id="checkinForm"
                            onsubmit="return handleAdminCheckInSubmit(event, {{ $booking->balance_due ?? 0 }})">
                            @csrf @method('PATCH')
                            <input type="hidden" name="status" value="checked_in">
                            <input type="hidden" name="balance_arrangement" id="checkinBalanceArrangement" value="">
                    ...
                    <div class="text-muted-theme" style="font-size:12px;background:#fef9c3;border-radius:8px;padding:10px 12px;">
                        <i class="bi bi-hourglass-split me-1"></i>
                     Awaiting payment from the guest — it will automatically become "Confirmed" once the payment is successful. No action is required unless you wish to cancel.
                    </div>
                    @endif
                </div>
            </div>
            ...
            <div class="card-panel">
                ...
                <div class="card-body-custom">
                    <p class="text-muted-theme mb-12" style="font-size:12px;">
                        Kasalukuyang check-out: <strong>{{ $booking->check_out_date->format('M d, Y') }}
                        @if($booking->check_out_time) — {{ \Carbon\Carbon::parse($booking->check_out_time)->format('g:i A') }}@endif</strong>.
                        Kung sasagutin ang extension request, che-check muna kung may susunod na guest na naka-schedule bago payagan.
                        <strong>Manual na idagdag ang extension fee sa "Extra Charges" sa ibaba</strong> kung meron.
                    </p>
                    <form method="POST" action="{{ route('admin.bookings.extend', $booking) }}">
                        ...
                        <div class="mb-12" style="display:grid;grid-template-columns:1fr 1fr;gap:12px;">
                            <div>
                                <label class="form-label-sm">Bagong Check-out Date *</label>
                                <input type="date" name="new_check_out_date" class="form-control-sm-custom"
                                    min="{{ $booking->check_out_date->format('Y-m-d') }}" required>
                            </div>
                            <div>
                                <label class="form-label-sm">Bagong Check-out Time *</label>
                                <input type="time" name="new_check_out_time" class="form-control-sm-custom"
                                    value="{{ $booking->check_out_time ? \Carbon\Carbon::parse($booking->check_out_time)->format('H:i') : '' }}" required>
                            </div>
                        </div>
                        <button type="submit" class="btn-status" style="background:var(--terracotta);color:#fff;width:100%;justify-content:center;">
                ...
                <div class="card-body-custom">
                    <div class="info-grid">
                        ...
                        <div class="info-item">
                            ...
                            <div class="value">{{ $booking->check_in_date->format('l, F j, Y') }} @if($booking->check_in_time)&nbsp;— {{ \Carbon\Carbon::parse($booking->check_in_time)->format('g:i A') }}@endif</div>
                        </div>
                        <div class="info-item">
                            <div class="label">Check-out Date</div>
                            <div class="value">{{ $booking->check_out_date->format('l, F j, Y') }} @if($booking->check_out_time)&nbsp;— {{ \Carbon\Carbon::parse($booking->check_out_time)->format('g:i A') }}@endif</div>
                        </div>
                        <div class="info-item">
                    ...
                    <div class="text-muted-theme" style="margin-top:16px;padding:12px 14px;background:#f8fafc;border-radius:8px;font-size:13px;">
            ...
            <div class="card-panel">
                ...
                <div class="card-body-custom">
                    @if($booking->payments->isEmpty())
                        <p class="text-muted-theme" style="font-size:13px;text-align:center;padding:20px 0;">No payments recorded yet.</p>
                    @else
            ...
            <div class="card-panel">
                ...
                <div class="card-body-custom">
                    @if($booking->extras->isEmpty())
                        <p class="text-muted-theme" style="font-size:13px;text-align:center;padding:10px 0;">Walang naitalang extra charge.</p>
                    @else
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        ...
                                        <div class="text-muted-theme" style="font-size:11px;">{{ $extra->description }}</div>
                                        @endif
                                    </td>
                                    <td class="text-center">{{ $extra->quantity }}</td>
                                    <td class="text-end">₱{{ number_format($extra->unit_price, 2) }}</td>
                                    <td style="text-align:right;font-weight:600;">₱{{ number_format($extra->total, 2) }}</td>
                                    <td class="text-end">
                                        <form method="POST" action="{{ route('admin.bookings.extras.destroy', [$booking, $extra]) }}"
                                              onsubmit="return confirm('Tanggalin ang extra charge na ito?');" style="display:inline;">
                                            @csrf @method('DELETE')
                                            <button type="submit" style="background:none;border:none;color:#ef4444;cursor:pointer;font-size:14px;" title="Remove">
                    ...
                    <form method="POST" action="{{ route('admin.bookings.extras.store', $booking) }}">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:10px;">
                            <div>
                                ...
                                <input type="text" name="item_name" class="form-control-sm-custom" placeholder="hal. Extra Grill Set, Karaoke Rental" required>
                            </div>
                            <div>
                                <label class="form-label-sm">Description (optional)</label>
                                <input type="text" name="description" class="form-control-sm-custom" placeholder="Detalye (opsyonal)">
                            </div>
                        </div>
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:10px;">
                            <div>
                                <label class="form-label-sm">Quantity *</label>
                                <input type="number" name="quantity" class="form-control-sm-custom" value="1" min="1" required>
                            </div>
                            <div>
                                <label class="form-label-sm">Unit Price (₱) *</label>
                                <input type="number" name="unit_price" class="form-control-sm-custom" step="0.01" min="0" placeholder="0.00" required>
                            </div>
                        </div>
                        <button type="submit" class="btn-status" style="background:var(--terracotta);color:#fff;width:100%;justify-content:center;">
            ...
            <div class="card-panel">
                ...
                <div class="card-body-custom">
                    @if($booking->property->primaryImage)
                        <img src="{{ $booking->property->primaryImage->url }}"
                             class="mb-12" style="width:100%;height:140px;object-fit:cover;border-radius:8px;" alt="">
                    @endif
...
<div class="modal fade" id="balanceConfirmModal" tabindex="-1">
    <div class="modal-dialog modal-dialog-centered" style="max-width:420px;">
        <div class="modal-content" style="border-radius:14px;border:none;">
            <div class="modal-body p-4">
                <h5 style="font-family:'Cormorant Garamond',serif;font-size:20px;margin-bottom:6px;">May Natitirang Balance</h5>
                <p style="font-size:13px;color:#64748b;margin-bottom:10px;">
                   This booking has a balance of
                    <strong style="color:#b45309;">₱{{ number_format($booking->balance_due ?? 0, 2) }}</strong>.
                 Would you like to record the payment now, or check in with deferred payment (to be paid upon check-out)?
                </p>
                ...
                <div style="border-top:1px solid var(--border);padding-top:14px;">
                    <label style="display:flex;align-items:flex-start;gap:8px;font-size:12.5px;color:#64748b;cursor:pointer;">
                        ...
                        I confirm that I will defer the payment of this balance until check-out. I take full responsibility for this decision.
                    </label>
                </div>
                <div style="display:flex;gap:8px;margin-top:14px;">
                    <button type="button" class="btn btn-light w-50" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" id="adminConfirmDeferredBtn" class="btn btn-danger w-50" disabled style="opacity:.5;"
                        onclick="confirmAdminDeferredCheckIn()">
