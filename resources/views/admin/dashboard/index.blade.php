...
@section('page-title', "Good " . (now()->hour < 12 ? 'Morning' : (now()->hour < 18 ? 'Afternoon' : 'Evening')) . ", " . explode(' ', auth()->user()->full_name)[0] . " 👋")
@section('page-subtitle', now()->format('l, F j, Y') . " ​·​ Here's what's happening at the resort today")
...
.kpi-card.teal::before   { background: linear-gradient(90deg, var(--teal, #14b8a6), #5ee7e7); }
.kpi-card.navy::before   { background: linear-gradient(90deg, var(--navy-mid, #1e3a5f), #3a6186); }
...
.custom-table tr:hover td { background: #faf7f2; }
...
.status-pending    { background: var(--tag-amber-bg); color: var(--tag-amber-fg); }
.status-confirmed  { background: var(--tag-green-bg); color: var(--tag-green-fg); }
.status-checked_in { background: var(--tag-blue-bg); color: var(--tag-blue-fg); }
.status-cancelled  { background: var(--tag-red-bg); color: var(--tag-red-fg); }
.status-checked_out{ background: var(--tag-slate-bg); color: var(--tag-slate-fg); }

.pay-paid     { background: var(--tag-green-bg); color: var(--tag-green-fg); }
.pay-partial  { background: var(--tag-amber-bg); color: var(--tag-amber-fg); }
.pay-unpaid   { background: var(--tag-red-bg); color: var(--tag-red-fg); }
.pay-refunded { background: var(--tag-cyan-bg); color: var(--tag-cyan-fg); }
