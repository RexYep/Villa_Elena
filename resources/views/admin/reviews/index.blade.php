                    ...
                    <div class="review-card-body">
                        ...
                        <div class="review-actions">
                            @if ($review->status === 'pending')
                                ...
                                <button type="button" class="btn-reject" onclick="openRejectModal({{ $review->id }})">
                                    <i class="bi bi-x-lg me-1"></i> Reject
                                </button>
                            @endif

                            @if ($review->status === 'approved' && !$review->admin_reply)
                                <button type="button" class="btn-reply" onclick="toggleReply({{ $review->id }})">
                        ...
                        <div class="reply-form" id="replyForm{{ $review->id }}">
                            <form method="POST" action="{{ route('admin.reviews.reply', $review) }}">
                                @csrf @method('POST')
                                <textarea name="reply" class="reply-input" rows="2" placeholder="Write a reply as Villa Elena Resort..."
