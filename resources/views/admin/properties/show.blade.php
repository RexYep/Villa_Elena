...
@push('styles')
<style>
.btn-navy{background:var(--terracotta);color:#fff;border:none;border-radius:9px;padding:9px 18px;font-size:13px;font-weight:600;cursor:pointer;font-family:'DM Sans',sans-serif;display:inline-flex;align-items:center;gap:6px;transition:all .2s;text-decoration:none;}
...
.status-available   {background:var(--tag-green-bg);color:var(--tag-green-fg);}
.status-occupied    {background:#fee2e2;color:#dc2626;}
.status-maintenance {background:var(--tag-amber-bg);color:var(--tag-amber-fg);}

/* CARDS */
.panel{background:var(--cream);border-radius:14px;border:1px solid var(--border);overflow:hidden;margin-bottom:20px;}
...
.panel-head h3{font-family:'Cormorant Garamond',serif;font-size:17px;font-weight:600;color:var(--stone);}
.panel-body{padding:22px;}

/* STAT MINI CARDS */
.mini-stats{display:grid;grid-template-columns:repeat(4,1fr);gap:16px;margin-bottom:20px;}
.mini-card{background:var(--cream);border-radius:12px;border:1px solid var(--border);padding:18px 20px;}
.mini-card .val{font-family:'Cormorant Garamond',serif;font-size:26px;font-weight:700;color:var(--stone);line-height:1;}
.mini-card .lbl{font-size:11px;color:var(--muted);margin-top:5px;text-transform:uppercase;letter-spacing:.5px;}
.mini-icon{width:36px;height:36px;border-radius:9px;display:flex;align-items:center;justify-content:center;font-size:16px;margin-bottom:12px;background:var(--gold-dim);color:var(--gold);}
...
.no-image{width:100%;height:130px;background:var(--sand);border-radius:10px;border:1px dashed var(--border);display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:13px;flex-direction:column;gap:6px;}

/* INFO GRID */
.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:16px;}
.info-item label{font-size:11px;font-weight:600;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;display:block;margin-bottom:4px;}
.info-item .val{font-size:14px;color:var(--text-main);font-weight:500;}

/* AMENITIES */
.amenity-tag{display:inline-flex;align-items:center;gap:5px;background:var(--sand);border-radius:20px;padding:4px 12px;font-size:12px;color:var(--text-main);margin:3px;}
...
@section('content')
    ...
    @if($property->type === 'villa')
        <div class="tag-green" style="border-radius:10px;padding:12px 16px;margin-bottom:18px;font-size:13px;">
            ...
            Ito ang <strong>master Villa record</strong> — ang tanging bookable na listing sa customer-facing site. Lahat ng booking, kita, at review ay naka-tally dito.
        </div>
    @elseif($property->type === 'room')
        <div style="background:#f1f5f9;color:#475569;border-radius:10px;padding:12px 16px;margin-bottom:18px;font-size:13px;">
            <i class="bi bi-info-circle me-2"></i>
            Ito ay isa sa mga <strong>kwarto ng Villa Elena</strong> — info/reference lang ito (status, litrato, housekeeping). Hindi ito hiwalay na binebentang listing, kaya inaasahang <strong>0 ang bookings/reviews</strong> dito — nasa master Villa record ang mga iyon.
        </div>
    @endif
    ...
    <div class="two-col">
        ...
        <div>
                ...
                <div class="panel-body">
                    ...
                    @if($property->description)
                        <div style="margin-top:18px; padding-top:18px; border-top:1px solid var(--border);">
                            <label class="text-muted-theme section-label" style="font-size:11px; display:block; margin-bottom:8px;">Description</label>
                    ...
                    @if(!empty($property->amenities))
                        <div style="margin-top:18px; padding-top:18px; border-top:1px solid var(--border);">
                            <label class="text-muted-theme section-label" style="font-size:11px; display:block; margin-bottom:8px;">Amenities</label>
                            <div>
                ...
                @else
                    <table>
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td>
                                    <a href="{{ route('admin.bookings.show', $booking) }}" style="font-weight:600; color:var(--stone); text-decoration:none; font-size:12px;">
                                        {{ $booking->booking_ref }}
                                    </a>
                                </td>
                                <td>{{ $booking->user->full_name ?? 'N/A' }}</td>
                                <td class="text-muted-theme" style="font-size:12px;">{{ $booking->check_in_date->format('M d, Y') }}</td>
                                <td class="text-muted-theme" style="font-size:12px;">{{ $booking->check_out_date->format('M d, Y') }}</td>
                                <td>
            ...
            <div class="panel">
                <div class="panel-head">
                    <h3>Reviews <span class="text-muted-theme" style="font-size:13px; font-weight:400;">({{ $property->reviews->count() }})</span></h3>
                </div>
                ...
                @else
                    <div class="panel-body" style="display:flex; flex-direction:column; gap:14px;">
                        ...
                        <div style="padding:14px; background:#f8fafc; border-radius:10px; border:1px solid var(--border);">
                            ...
                            @if($review->comment)
                                <p class="text-muted-theme" style="font-size:13px; line-height:1.5;">{{ $review->comment }}</p>
                            @endif
            ...
            <div class="panel" id="blockDatesPanel">
                ...
                <div class="panel-body">
                    ...
                    <p class="text-muted-theme mb-12" style="font-size:12px;">
                        Ang pag-block ng petsa dito ay direktang makakaapekto sa availability na makikita ng customer (dahil ito ang master, bookable na Villa).
                    </p>
                    @else
                    <p class="text-muted-theme mb-12" style="font-size:12px;">
                        <i class="bi bi-info-circle"></i> Note: internal tracking/reference lang ito para sa kwartong ito. Hindi ito direktang nakaka-apekto sa availability ng buong Villa sa customer-facing site — para blockin ang aktwal na petsa ng booking, i-block ito sa master Villa record.
                    </p>
                    @endif
                    <form method="POST" action="{{ route('admin.properties.block', $property) }}"
                        ...
                        <div style="margin-bottom:14px;">
                            <label style="font-size:11px; font-weight:600; color:#374151; display:block; margin-bottom:5px;">Notes <span class="text-muted-theme" style="font-weight:400;">(optional)</span></label>
                    ...
                    @if($property->availabilityBlocks->count() > 0)
                        <div style="margin-top:18px; padding-top:18px; border-top:1px solid var(--border);">
                            <div class="text-muted-theme section-label" style="font-size:11px; margin-bottom:10px;">Blocked Periods</div>
