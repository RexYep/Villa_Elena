...
.stat-chip{background:var(--cream);border-radius:10px;padding:14px 16px;border:1px solid var(--border);cursor:pointer;transition:all .2s;text-decoration:none;display:block;}
.stat-chip:hover{border-color:var(--terracotta);box-shadow:0 2px 8px rgba(44,36,22,.09);}
.stat-chip.active{background:var(--terracotta);border-color:var(--terracotta);}
.stat-chip .val{font-family:'Cormorant Garamond',serif;font-size:26px;font-weight:700;color:var(--text-main);line-height:1;}
.stat-chip.active .val{color:#fff !important;}
.stat-chip .lbl{font-size:11px;color:var(--muted);margin-top:3px;}
.stat-chip.active .lbl{color:rgba(255,255,255,0.75);}

.filters-bar{background:var(--cream);border-radius:12px;border:1px solid var(--border);padding:14px 20px;margin-bottom:20px;display:flex;gap:10px;flex-wrap:wrap;align-items:center;}
.filter-input{border:1.5px solid var(--border);border-radius:8px;padding:8px 12px;font-size:13px;font-family:'DM Sans',sans-serif;background:#fff;color:var(--text-main);}
...
.status-active  {background:var(--tag-green-bg);color:var(--tag-green-fg);}
.status-inactive{background:var(--tag-red-bg);color:var(--tag-red-fg);}
...
@section('content')
    ...
    {{-- Stats --}}
    <div class="stats-row">
        <a href="{{ route('admin.users.index') }}" class="stat-chip {{ !request('role') && !request()->has('status') ? 'active' : '' }}">
