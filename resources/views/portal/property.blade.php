...
/* ── Gallery ── */
.gallery{display:grid;grid-template-columns:2.1fr 1fr;grid-template-rows:1fr 1fr;gap:12px;border-radius:20px;overflow:hidden;height:100%;min-height:520px;}
.gallery-main{grid-row:1/-1;position:relative;height:100%;}
.gallery-main:only-child{grid-column:1/-1;}
.gallery-img{width:100%;height:100%;object-fit:cover;display:block;transition:transform 0.4s ease;}
.gallery > div{overflow:hidden;position:relative;}
.gallery > div:hover .gallery-img{transform:scale(1.03);}
...
.gallery-count{position:absolute;bottom:14px;right:14px;background:rgba(0,0,0,.7);backdrop-filter:blur(4px);color:#fff;padding:6px 14px;border-radius:100px;font-size:12.5px;font-weight:500;}

/* ── Top Grid: Gallery + Availability (side by side) ── */
.top-grid{display:grid;grid-template-columns:1.35fr 1fr;gap:24px;align-items:stretch;margin-bottom:36px;}
.availability-card{background:#fff;border:1px solid var(--border);border-radius:20px;padding:24px;box-shadow:0 8px 32px rgba(44,36,22,.06);}
.availability-card-title{font-family:'Playfair Display',serif;font-size:19px;font-weight:600;margin-bottom:4px;}
.availability-card-sub{font-size:12.5px;color:var(--muted);margin-bottom:14px;line-height:1.5;}
...
.fc-wrap{background:#fff;border:1px solid var(--border);border-radius:16px;padding:16px;overflow:visible;}
.fc{font-family:'Jost',sans-serif;}
.fc .fc-toolbar-title{font-family:'Playfair Display',serif;font-size:16px;color:var(--stone);}
...
.fc .fc-button{background:var(--sand);border:1px solid var(--border);color:var(--stone);box-shadow:none;text-transform:capitalize;font-size:12px;padding:4px 10px;}
.fc .fc-button:hover{background:var(--gold-light);color:#fff;}
.fc .fc-button-primary:not(:disabled).fc-button-active,
.fc .fc-button-primary:not(:disabled):active{background:var(--stone);border-color:var(--stone);color:#fff;}
.fc .fc-daygrid-day.fc-day-today{background:rgba(184,148,63,.08);}
/* Make day numbers more visible */
.fc .fc-daygrid-day-number{font-size:13px;font-weight:600;padding:6px 8px;color:var(--stone);}
.fc .fc-col-header-cell-cushion{font-size:12px;font-weight:600;text-transform:uppercase;letter-spacing:0.5px;padding:8px 4px;}
.fc .fc-daygrid-day-frame{min-height:48px;}
.fc-event.booked-event{background:#dc2626;border:none;color:#fff;font-size:10px;padding:1px 4px;cursor:default;border-radius:4px;}
...
.fc-list-event.booked-event td{background:#dc2626 !important;color:#fff !important;}
.fc-list-event.booked-event:hover td{background:#a91d1d !important;color:#fff !important;}
.fc-list-event.booked-event .fc-list-event-title,
.fc-list-event.booked-event .fc-list-event-time{color:#fff !important;}
.cal-legend{display:flex;gap:16px;font-size:12px;color:var(--muted);margin-top:14px;}
...
.duration-note{font-size:12px;margin-top:6px;display:none;padding:8px 10px;border-radius:8px;}
.duration-note.ok{display:block;background:#dcfce7;color:#15803d;}
.duration-note.warn{display:block;background:#fee2e2;color:#dc2626;}
.slot-options{display:grid;grid-template-columns:1fr 1fr;gap:10px;}
.slot-option{position:relative;display:block;border:1px solid var(--border);border-radius:10px;padding:10px 12px;cursor:pointer;transition:all .15s;}
.slot-option input{position:absolute;top:10px;right:10px;margin:0;}
.slot-option-label strong{display:block;font-size:13px;color:var(--stone);}
.slot-option-label small{display:block;font-size:11px;color:var(--muted);margin-top:2px;line-height:1.4;}
.slot-option:has(input:checked){border-color:var(--gold);background:rgba(184,148,63,.06);}
.price-preview .estimate-tag{font-size:10px;letter-spacing:1px;text-transform:uppercase;color:var(--muted);margin-bottom:6px;display:block;}
.price-preview .estimate-disclaimer{font-size:11px;color:var(--muted);margin-top:8px;line-height:1.5;}
.popover{font-family:'Jost',sans-serif;max-width:240px;}
.popover-header{font-family:'Playfair Display',serif;font-size:13px;background:var(--stone);color:#fff;}
.popover-body{font-size:12px;}

@media(max-width:900px){
    .detail-grid{grid-template-columns:1fr;}
    .top-grid{grid-template-columns:1fr;}
    .gallery{grid-template-columns:1fr 1fr;grid-template-rows:240px 140px;min-height:auto;height:auto;}
    .gallery-main{grid-row:1/-1;}
    .booking-card{position:static;}
}
@media(max-width:480px){
    .prop-name{font-size:26px;}
    .slot-options{grid-template-columns:1fr;}
    .gallery{grid-template-columns:1fr;grid-template-rows:200px 120px 120px;}
        ...
        <div>
            <div class="booking-card">
                @if($property->status !== 'maintenance')
                    ...
                    <div class="text-muted-theme" style="font-size:11px;margin-bottom:2px;">Day (8AM–5PM) or Night (7PM–6AM) whole-villa stay</div>
                    @if($property->weekend_price && $property->weekend_price != $property->base_price)
                    <div class="price-weekend">₱{{ number_format($property->weekend_price,0) }} peak package (Fri–Sun before 6PM)</div>
                    @endif
                        ...
                        <div class="mb-12">
                            ...
                            <div class="slot-options">
                                <label class="slot-option">
                                    <input type="radio" name="slot" value="day" id="slot_day" {{ ($slot ?? 'day') === 'day' ? 'checked' : '' }}>
                                ...
                                <label class="slot-option">
                                    <input type="radio" name="slot" value="night" id="slot_night" {{ ($slot ?? 'day') === 'night' ? 'checked' : '' }}>
                                    <span class="slot-option-label">
                                        <strong>Night</strong>
                    ...
                    <div class="text-muted-theme" style="margin-top:16px;padding-top:16px;border-top:1px solid var(--border);font-size:12px;">
...
function showSlotNote() {
    ...
    if (!slot) { note.className = 'duration-note'; note.textContent = ''; return; }
...
function fetchServerPreview(ci, slot) {
    ...
    const params = new URLSearchParams({ checkin: ci, slot: slot });
    ...
    fetch(`${PRICE_PREVIEW_URL}?${params.toString()}`, {
        ...
        .then(data => {
            ...
            document.getElementById('previewBase').textContent  = data.price_formatted;
            document.getElementById('previewTotal').textContent = data.price_formatted;
            const bookBtn = document.getElementById('bookBtn');
            if (bookBtn) { bookBtn.disabled = false; bookBtn.textContent = 'Reserve Now →'; }
        })
        .catch(err => {
...
function updatePreview() {
    ...
    if (!ci || !slot) { document.getElementById('pricePreview').style.display = 'none'; return; }
...
const calendarEvents = bookedRangesRaw.map((r, i) => {
    ...
    if (startHM && endHM) {
        return {
            ...
            extendedProps: { rangeIndex: i, booking_id: r.id }
        };
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (PUSHER_KEY && window.Pusher) {
        const pusher  = new Pusher(PUSHER_KEY, { cluster: '{{ env('PUSHER_APP_CLUSTER', 'ap1') }}' });
