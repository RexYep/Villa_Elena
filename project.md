...
**Gallery restyled (same v7.18 pass).** The slideshow keeps its 7 photos, lightbox, hover/offscreen/hidden-tab pause and JS module, but: prev/next moved out of the photo into the `.section-header` row (cream "previous", gold "next"); the frame spans the section at 16:9 (16:10 ≤768px, 4:3 ≤480px), capped at `76vh` so a laptop screen shows it whole, with 32px corners; slides cross-fade over 0.7s while the photo settles from `scale(1.06)` to `1` over 4.5s; each slide carries a counter pill ("03 / 07"), title and a one-line `desc` in `$galleryShots`; dots sit under the frame with the current one stretched into a gold pill (`aria-current`). **The progress bar is gone and the interval is 5s (was 2.5s)** — the 4.5s settle needs to finish before the cut. The descriptions were written from the photos themselves: "Inside View" (`view-inside.jpg`) is the daytime pool courtyard, not a room, so keep that in mind if the captions are ever reworded. `#gallery .section` also gets a 20px phone gutter at ≤768px; its ID selector outranked the shared `.section` rule, so on phones the gallery alone kept the 40px desktop gutter.
